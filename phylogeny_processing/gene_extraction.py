...
fasta_dir = '/data/internship_data/nidhi/aba/new_output/nextflow_output/assemblies'
amrfinder_results_dir = '/data/internship_data/nidhi/aba/new_output/amrfinder_output'
output_dir = '/data/internship_data/nidhi/aba/new_output/extraction'
gene_names = ['blaNDM-1','blaOXA-23','blaOXA-66']
...
def find_contig_from_amrfinder(gene_name, amrfinder_result_filepath):
    with open(amrfinder_result_filepath) as tsvfile:
        ...
        for row in reader:
            if gene_name == row['Element symbol']:
...
for report_file in glob.glob(os.path.join(amrfinder_results_dir, "*.tsv")):
    # get file id
    sample_id = os.path.basename(report_file).replace('_amrfinder.tsv', '')
    ...
    fasta_matches = glob.glob(os.path.join(fasta_dir, f"{sample_id}*.fasta"))

    if not fasta_matches:
      print(f"No assembly found for {sample_id}")
      continue

    fasta_filepath = fasta_matches[0]
